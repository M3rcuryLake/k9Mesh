...
# paho-mqtt 2.x compatibility (CallbackAPIVersion required in 2.x, not available in 1.x)
try:
...
def deploy_code(args):
    ...
    try:
        ...
        if health.returncode != 0 or 'MP_OK' not in (health.stdout or ''):
            ...
            print(f"  {Fore.GREEN}./me flash --erase --chip c5{Style.RESET_ALL}")
...
def detect_live_motion(args):
    """Run live ML motion detection on PC from the UDP CSI stream."""
    parent_dir = Path(__file__).parent
    if str(parent_dir) not in sys.path:
        sys.path.insert(0, str(parent_dir))
    ...
    try:
        ...
        from src.ml_detector import MLDetector, ML_DEFAULT_THRESHOLD, ML_METRIC_SCALE, FEATURE_NAMES
    ...
    def on_packet(pkt):
        ...
        if args.log_turbulence:
            raw_turbulence = detector._context._compute_spatial_turbulence_in_buffer(
                pkt.iq_raw, subcarriers
            )
        else:
            raw_turbulence = None
        ...
        if should_publish:
            ...
            if should_log_publish(effective_state):
                ...
                if args.log_turbulence:
                    ...
                    print(
                        f"  turbulence: raw={raw_turbulence:.4f} "
                        f"filtered={filtered_turbulence:.4f}"
                    )
                    if window_tail:
...
Examples:
  ...
  ./me
    ...
    collect_parser.add_argument('--duration', '-d', type=float, default=2.0,
                               help='Duration per sample in seconds (default: 2.0)')
    ...
    collect_parser.add_argument('--bind-ip', default=None,
    ...
    detect_parser = subparsers.add_parser('detect', help='Run live ML motion detection from CSI UDP stream')
    detect_parser.add_argument('--udp-port', type=int, default=5001,
                              help='UDP port for CSI reception (default: 5001)')
    detect_parser.add_argument('--bind-ip', default=None,
                              help='Local IP/interface for UDP bind (default: auto-detect)')
    detect_parser.add_argument('--log-features', action='store_true',
                              help='Print the 9 ML features after each published sample')
    detect_parser.add_argument('--log-turbulence', action='store_true',
                              help='Print raw/filtered turbulence and recent buffer tail after each publish')
    detect_parser.add_argument('--log-only-motion', action='store_true',
...
class EspectreCLI_MQTT:
    ...
    def cmd_set_value(self, cmd_name, args, value_type, usage_hint):
        """Generic handler for commands that set a single value."""
        if not args:
